...
__all__ = ("SQLSpecMFALoginChallengeStore", "SQLSpecRecoveryCodeStore", "SQLSpecStepUpStore")
...
class SQLSpecMFALoginChallengeStore:
    ...
    async def put(self, challenge: "MFALoginChallenge") -> "None":
        ...
        col_epoch = quote_identifier(resolve_column(self._backend.config, TABLE_MFA_LOGIN_CHALLENGES, "security_epoch"))
        col_client_key = quote_identifier(
            resolve_column(self._backend.config, TABLE_MFA_LOGIN_CHALLENGES, "client_key")
        )
        col_issued_at = quote_identifier(resolve_column(self._backend.config, TABLE_MFA_LOGIN_CHALLENGES, "issued_at"))
...
class SQLSpecStepUpStore:
    ...
    async def put(self, record: "StepUpGrantState") -> "None":
        ...
        col_grant_digest = quote_identifier(resolve_column(self._backend.config, TABLE_STEP_UP_GRANTS, "grant_digest"))
        ...
        async with self._lock, self._backend.session() as session:
            ...
            for code in codes:
                if code.account_id == account_id:
                    await session.execute(
                        insert_query, str(uuid4()), account_id, code.pepper_version, code.digest, now_iso
                    )
