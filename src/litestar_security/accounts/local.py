...
if TYPE_CHECKING:
    from litestar_security.accounts.sessions import SessionBindingConfig
...
UserT = TypeVar("UserT")
LocalAccountCapabilities: TypeAlias = object
...
class LocalAuthConfig(Generic[UserT]):
    ...
    mode: LocalAuthMode
    accounts: LocalAccountCapabilities = field(repr=False)
    ...
    def __post_init__(self) -> None:
        ...
        if not route_prefix.startswith("/") or route_prefix == "":
        ...
        if self.mode in {LocalAuthMode.SESSION, LocalAuthMode.HYBRID} and (self.csrf is None or self.binding is None):
        ...
        if self.mode in {LocalAuthMode.TOKENS, LocalAuthMode.HYBRID}:
            audience = self.token_audience.strip() if self.token_audience is not None else ""
            if self.key_ring is None or not audience:
    ...
    def _validate_capabilities(self) -> None:
        from litestar_security.accounts.sessions import RefreshTokenFamilyStore, SessionRegistry
...
class LocalAuth:
    ...
    def session(
        cls,
        *,
        accounts: LocalAccountCapabilities,
        csrf: CSRFConfig | ExternalCSRF,
        binding: SessionBindingConfig,
        registration: RegistrationPolicy = RegistrationPolicy.disabled(),
